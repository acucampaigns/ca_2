...
function php_date(k, l) {
    var d, a, h = "Sun Mon Tues Wednes Thurs Fri Satur January February March April May June July August September October November December".split(" "), f = /\\?(.?)/gi, g = function (b, c) {
        return a[b] ? a[b]() : c
    }, e = function (b, a) {
        for (b = String(b); b.length < a;)b = "0" + b;
        return b
    };
    a = {
        d: function () {
            ...
            var b = a.j(), c = b % 10;
            3 >= c && 1 == parseInt(b % 100 / 10, 10) && (c = 0);
            return ["st", "nd", "rd"][c - 1] || "th"
        }, w: function () {
            return d.getDay()
        }, z: function () {
            ...
            return e(1 + Math.round((b - c) / 864E5 / 7), 2)
        }, F: function () {
            return h[6 + a.n()]
        }, m: function () {
            return e(a.n(), 2)
            ...
            var b = a.n(), c = a.W();
            return a.Y() + (12 === b && 9 > c ? 1 : 1 === b && 9 < c ? -1 : 0)
        }, Y: function () {
            ...
            var a = 3600 * d.getUTCHours(), c = 60 * d.getUTCMinutes(), f = d.getUTCSeconds();
            return e(Math.floor((a + c + f + 3600) / 86.4) % 1E3, 3)
        }, g: function () {
            return a.G() % 12 || 12
        }, G: function () {
            return d.getHours()
            ...
            return e(1E3 * d.getMilliseconds(), 6)
        }, e: function () {
            throw"Not supported (see source code of date() for timezone on how to add support)";
        }, I: function () {
            var b = new Date(a.Y(), 0), c = Date.UTC(a.Y(), 0), d = new Date(a.Y(), 6), e = Date.UTC(a.Y(), 6);
            return b - c !== d - e ? 1 : 0
        }, O: function () {
            var a = d.getTimezoneOffset(), c = Math.abs(a);
            return (0 < a ? "-" : "+") + e(100 * Math.floor(c / 60) + c % 60, 4)
        }, P: function () {
            var b = a.O();
    ...
    this.date = function (a, c) {
        d = void 0 === c ? new Date : c instanceof Date ? new Date(c) : new Date(1E3 * c);
        return a.replace(f, g)
    };
    return this.date(k, l)
}
...
$(function () {
    ...
    var imagesUrl = '/template/img/icons/';

    function getDiffInYears(startDate, endDate) {
        var yearsDiff = endDate.getFullYear() - startDate.getFullYear();
        var monthsDiff = endDate.getMonth() - startDate.getMonth();
        if (monthsDiff < 0 || (monthsDiff === 0 && endDate.getDate() < startDate.getDate())) {
            yearsDiff--;
        }
        return yearsDiff;
    }

    $.each(['valid', 'error'], function (i, type) {
        var image = new Image();
        image.src = imagesUrl + type + '_ico.png';
    });
    ...
    function onModalShown() {
        if (ir && $.isFunction(ir.messageParent)) {
            ir.messageParent(Math.floor($(this).offset().top));
        }
    }

    function showAlert(message, title) {
        if (!modals.alert.length) {
            modals.alert = $([
                '<div class="modal hide fade">',
                    ...
                    '<div class="modal-footer">',
                        '<a href="#" class="btn btn-primary" data-dismiss="modal">' + (jsData.translations.ok || 'OK') + '</a>',
                    '</div>',
                '</div>'
            ].join(''));
            $('body').append(modals.alert);
            modals.alert.on('shown', onModalShown);
        }
        modals.alert.find('.modal-header h3').html(title);
        modals.alert.find('.modal-body p').html(message);
        modals.alert.modal('show');
    }

    function showConfirm(message, title, callback) {
        if (!modals.confirm.length) {
            modals.confirm = $([
                '<div class="modal hide fade">',
                    '<div class="modal-header">',
                        '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>',
                    ...
                    '</div>',
                    '<div class="modal-footer">',
                        '<a href="#" class="btn btn-primary btn-continue">' + (jsData.translations.yes || 'Yes') + '</a>',
                        '<a href="#" class="btn" data-dismiss="modal">' + (jsData.translations.no || 'No') + '</a>',
                    '</div>',
                '</div>'
            ].join(''));
            $('body').append(modals.confirm);
            modals.confirm.on('shown', onModalShown);
        }
        modals.confirm.find('.modal-header h3').html(title);
        modals.confirm.find('.modal-body p').html(message);
        modals.confirm.find('.modal-footer .btn-continue').off('click').on('click', function (e) {
            e.preventDefault();
            if ($.isFunction(callback)) {
                callback.apply(modals.confirm);
            } else {
    ...
    $('.btn-submit').on('click', function (e) {
        ...
        var callback = function () {
            ...
            form.find('[name="action"]').val($.trim(el.data('action')));
            form.submit();
        };
        var message = $.trim(el.data('confirm'));
        if (message !== '') {
    ...
    (function () {
        ...
        if (applyForm.length) {
            ...
            var resetFieldIfHidden = function (field, newValue) {
                if (!newValue) {
                    this.$set(field, '');
                }
            };

            var isYes = function (value) {
                return value + '' === '1';
            };
            ...
            var validatorScope = Object.create(null);

            var validators = {
                required: function (value) {
                    return $.trim(value) !== '';
                },
                number: function (value, allowSpace) {
                    return (allowSpace ? /^[\d ]*$/ : /^\d*$/).test(value);
                },
                ...
                propercase: function (value) {
                    return /^$|^[a-zA-ZáàâäéèêëíìîïóòôöúùûüçÁÀÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÇ\s\-']*$/.test(value);
                },
                email: function (value) {
                    return /^[éàèùâêîôûëïöüæçœÉÀÈÙÂÊÎÔÛËÏÖÜÆÇŒa-zA-Z0-9_\.-]+@[éàèùâêîôûëïöüæçœÉÀÈÙÂÊÎÔÛËÏÖÜÆÇŒa-zA-Z0-9_\.-]+\.[éàèùâêîôûëïöüæçœÉÀÈÙÂÊÎÔÛËÏÖÜÆÇŒa-zA-Z_-]{2,6}$/.test(value);
                },
                gcmstext: function (value) {
                    return /^[0-9a-zA-ZáàâäéèêëíìîïóòôöúùûüçÁÀÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÇ#\-,.()'\/&*@:;"\s]*$/.test(value);
                },
                alphanumeric: function (value, allowSpace) {
                    return (allowSpace ? /^[a-zA-ZáàâäéèêëíìîïóòôöúùûüçÁÀÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÇ0-9\s]*$/ : /^[a-zA-ZáàâäéèêëíìîïóòôöúùûüçÁÀÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÇ0-9]*$/).test(value);
                },
                usaprcard: function (value) {
                ...
                postalcode: function (value) {
                    for (var i = 0; i < postalCodePatterns.length; i++) {
                        if (postalCodePatterns[i].test(value)) {
                            return true;
                        }
                    }
                    return false;
            ...
            var Validation = function (directive) {
                this.el = directive.el;
                this.$el = $(directive.el);
                this.isCheckbox = this.$el.is(':checkbox');
                this.scope = directive.vm;
                if (_.has(this.scope, 'item') && _.has(this.scope, 'index')) {
                    this.scope = this.scope.item;
                }
                this.model = Vue.util.camelize(directive.validationName);
                ...
                if (directive.modifiers) {
                    _.each(directive.modifiers, function (value, name) {
                        if (name == 'date') {
                            this.isDate = true;
                            return;
                        }
            ...
            Validation.prototype.getValue = function () {
                return this.isCheckbox ? (this.$el.prop('checked') ? 1 : '') : this.el.value;
            };

            Validation.prototype.validate = function (forceDirty) {
                var isValid = true;
                var value = this.getValue();
                if (forceDirty || (!this.dirty && value !== this.init)) {
                    this.dirty = true;
                }
                var validators = _.keys(this.validators);
                var count = validators.length;
                if (count) {
                    for (var i = 0; i < count; i++) {
                        var validator = this.validators[validators[i]];
                        isValid = validator.fn(value, validator.params);
                        if (!isValid) {
                            break;
                        }
                    }
                }
                Vue.util.set(validatorScope, this.model, {
                    valid: isValid,
                    invalid: !isValid,
                    dirty: this.dirty
                });
            ...
            Validation.prototype.configure = function (config) {
                _.each(config, function (params, validator) {
                    if (validator == 'date') {
                        if (this.isDate && _.isObject(params) && _.has(params, 'name')) {
                            this.dateParams = params;
                            var property = Object.create(null);
                            Vue.util.set(validatorScope, params.name, property);
                            Object.defineProperty(property, 'valid', {
                                enumerable: true,
                                configurable: true,
                                get: _.bind(function () {
                                    var isValid = true;
                                    var parts = [];
                                    var partsFormatted = [];
                                    _.each(params.parts, function (part) {
                                        var value = parseInt(this.scope[part]);
                                        if (isNaN(value) || value < 0) {
                                            value = 0;
                                            isValid = false;
                                        }
                                        isValid = isValid && validatorScope[part].valid;
                                        partsFormatted.push((value < 10 ? '0' : '') + value);
                                        parts.push(value);
                                    }, this);
                                    if (isValid) {
                                        var date = partsFormatted.join('-');
                                        if (date !== php_date('Y-m-d', new Date(parts[0], parts[1] - 1, parts[2], 0, 0, 0))) {
                                            isValid = false;
                                        } else {
                                            if (params.past) {
                                                if (date > php_date('Y-m-d')) {
                                                    isValid = false;
                                                }
                                            } else {
                                                if (params.future) {
                            ...
                            Object.defineProperty(property, 'dirty', {
                                enumerable: true,
                                configurable: true,
                                get: function () {
                                    var isDirty = false;
                                    _.each(params.parts, function (part) {
                                        isDirty = isDirty || validatorScope[part].dirty;
                                    });
                                    return isDirty;
                                }
                            });
                            Object.defineProperty(property, 'invalid', {
                                enumerable: true,
                                configurable: true,
                                get: function () {
                                    return !this.valid;
                        ...
                        if (_.has(this.validators, validator)) {
                            this.validators[validator].params = params;
                        }
                    }
                }, this);
            };
            ...
            Validation.prototype.destroy = function () {
                Vue.util.del(validatorScope, this.model);
                if (this.isDate && this.dateParams) {
                    Vue.util.del(validatorScope, this.dateParams.name);
                }
            };
            ...
            Vue.directive('validate', {
                ...
                bind: function () {
                    var self = this;
                    if (this._host) {
                        this._host.$on('hook:destroyed', function () {
                            self.unbind();
                        });
                    }
                    this.validationName = this.params.vValidation || this.arg.replace(/\$index/g, this.params.iIndex || 0);
                    var validation = new Validation(this);
                    validations[this.validationName] = validation;
                    this.on('change', function () {
                        validation.validate();
                    });
                },
                update: function (value) {
                    if (!_.has(validations, this.validationName)) {
                        return;
                    }
                    if (value) {
                        validations[this.validationName].configure(value);
                    }
                    validations[this.validationName].validate();
                },
                unbind: function () {
                    if (_.has(validations, this.validationName)) {
                        validations[this.validationName].destroy();
                    }
                    delete validations[this.validationName];
                }
            });
            ...
            Vue.directive('popover', {
                bind: function () {
                    var el = $(this.el);
                    var content = $.trim(el.data('content'));
                    if (content === '') {
                        var container = el.closest('.add-on');
                        (container.length ? container : el).hide();
                        return;
                    }
                    el.popover();
                },
            ...
            Vue.component('validation-status', {
                template: $('#template-validation-status').html(),
                props: {
                    isValid: {}
                },
                computed: {
                    iconUrl: function () {
                        return imagesUrl + (this.isValid ? 'valid' : 'error') + '_ico.png';
                    }
                }
            });

            var NotEligibleResult = Vue.extend({
                template: $('#template-not-eligible-result').html()
            });

            var ExemptResult = Vue.extend({
                template: $('#template-exempt-result').html()
            });

            var application = jsData.application || {};
            var employment = application.employment || {};

            var recordGenerators = {
                otherCitizenship: function () {
                    return {
                        id: _.uniqueId('f'),
                        citizenshipCountry: ''
                    };
            ...
            var ApplicationOtherCitizenship = Vue.extend({
                template: $('#template-application-other-citizenship').html(),
                props: [
                    'item',
                    'index',
                    'total'
                ...
                computed: {
                    citizenshipCountryOptions: function () {
                        var exclude = [];
                        if (this.$root.citizenshipCountry > 0) {
                            exclude.push(parseInt(this.$root.citizenshipCountry, 10));
                        }
                        var otherCitizenships = [];
                        var nextStep = this.$root.$refs.nextStep;
                        if (nextStep instanceof ApplicationForm) {
                            otherCitizenships = nextStep.otherCitizenships;
                        } else {
                            if (nextStep instanceof RepresentativeForm && nextStep.$refs.applicationForm instanceof ApplicationForm) {
                                otherCitizenships = nextStep.$refs.applicationForm.otherCitizenships;
                            }
                        }
                        _.each(otherCitizenships, function (otherCitizenship) {
                            if (otherCitizenship.id == this.item.id || !otherCitizenship.citizenshipCountry) {
                                return;
                            }
                            exclude.push(parseInt(otherCitizenship.citizenshipCountry, 10));
                        }, this);
                        var countries = jsData.dictionaries ? jsData.dictionaries.citizenshipCountries : [];
                        if (!exclude.length) {
                            return countries;
                        }
                        return _.filter(countries, function (country) {
                            return !_.contains(exclude, country.id);
                        });
                    },
                    citizenshipCountryValidationKey: function () {
                        return 'otherCitizenship' + Vue.util.camelize('-' + this.item.id) + 'CitizenshipCountry';
                    }
                },
                methods: {
                    addAnother: function () {
                        this.$dispatch('other-citizenship-add-another');
                    },
                    removeAnother: function () {
                        this.$dispatch('other-citizenship-remove-another', this.item);
                    }
                }
            });

            var ApplicationForm = Vue.extend({
                template: $('#template-application-form').html(),
                components: {
                    otherCitizenship: ApplicationOtherCitizenship
                },
                data: function () {
                    return {
                        currentYear: new Date().getFullYear(),
                        birthCountryOptions: jsData.dictionaries ? jsData.dictionaries.birthCountries : [],
                        genderOptions: jsData.dictionaries ? jsData.dictionaries.genders : [],
                        maritalStatusOptions: jsData.dictionaries ? jsData.dictionaries.maritalStatuses : [],
                        availableFundsOptions: jsData.dictionaries ? jsData.dictionaries.availableFunds : [],
                        countryOfIssuanceOptions: jsData.dictionaries ? jsData.dictionaries.passportIssueCountries : [],
                        occupationOptions: jsData.dictionaries ? jsData.dictionaries.occupations : [],
                        jobTitleOptions: jsData.dictionaries ? jsData.dictionaries.jobTitles : [],
                        countryOptions: jsData.dictionaries ? jsData.dictionaries.countries : [],
                        provinceOptions: jsData.dictionaries ? jsData.dictionaries.provinces : [],
                        stateOptions: jsData.dictionaries ? jsData.dictionaries.states : [],
                        preferredLanguageOptions: jsData.dictionaries ? jsData.dictionaries.preferredLanguages : [],
                        monthOptions: jsData.dictionaries ? jsData.dictionaries.months : [],
                        lastName: application.lastName || '',
                        firstName: application.firstName || '',
                        dobDay: application.dobDay || '',
                        dobMonth: application.dobMonth || '',
                        dobYear: application.dobYear || '',
                        countryOfBirth: application.countryOfBirth || '',
                        cityTownOfBirth: application.cityTownOfBirth || '',
                        hasOtherCitizenship: application.hasOtherCitizenship || '',
                        otherCitizenships: _.isArray(application.otherCitizenships) && application.otherCitizenships.length ? application.otherCitizenships : [recordGenerators.otherCitizenship()],
                        taiwanIDNumber: application.taiwanIDNumber || '',
                        usaResidentCardNumber: application.usaResidentCardNumber || '',
                        gender: application.gender || '',
                        maritalStatus: application.maritalStatus || '',
                        hasPreviouslyAppliedToCanada: application.hasPreviouslyAppliedToCanada || '',
                        uci: application.uci || '',
                        availableFunds: application.availableFunds || '',
                        passportNumber: application.passportNumber || '',
                        countryOfIssuance: application.countryOfIssuance || '',
                        issueDateDay: application.issueDateDay || '',
                        issueDateMonth: application.issueDateMonth || '',
                        issueDateYear: application.issueDateYear || '',
                        expiryDateDay: application.expiryDateDay || '',
                        expiryDateMonth: application.expiryDateMonth || '',
                        expiryDateYear: application.expiryDateYear || '',
                        employment: {
                            occupation: employment.occupation || '',
                            title: employment.title || '',
                            companyEmployerSchoolFacilityName: employment.companyEmployerSchoolFacilityName || '',
                        ...
                        languageOfPreference: application.languageOfPreference || '',
                        emailAddress: application.emailAddress || '',
                        aptUnit: application.aptUnit || '',
                        streetNo: application.streetNo || '',
                        streetAddress: application.streetAddress || '',
                        streetAddressAlt: application.streetAddressAlt || '',
                        ...
                        refusedVisaOrPermitOrDeniedEntryToCanada: application.refusedVisaOrPermitOrDeniedEntryToCanada || '',
                        refusedVisaOrPermitOrDeniedEntryToCanadaDetails: application.refusedVisaOrPermitOrDeniedEntryToCanadaDetails || '',
                        committedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhere: application.committedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhere || '',
                        committedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhereDetails: application.committedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhereDetails || '',
                        youOrFamilyMemberInContactWithPersonWithTuberculosis: application.youOrFamilyMemberInContactWithPersonWithTuberculosis || '',
                        youOrFamilyMemberInContactWithPersonWithTuberculosisDetails: application.youOrFamilyMemberInContactWithPersonWithTuberculosisDetails || '',
                        haveHealthConditionReceivingRegularAndOngoingMedicalTreatment: application.haveHealthConditionReceivingRegularAndOngoingMedicalTreatment || '',
                        haveKidneyConditionRequiringDialysis: application.haveKidneyConditionRequiringDialysis || '',
                        travellingForMedicalReasonsOrTreatment: application.travellingForMedicalReasonsOrTreatment || '',
                        haveFinancialArrangementsToCoverTreatmentAndRelatedExpenses: application.haveFinancialArrangementsToCoverTreatmentAndRelatedExpenses || '',
                        haveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonths: application.haveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonths || '',
                        haveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonthsDetails: application.haveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonthsDetails || '',
                        haveOrWillHaveHealthInsuranceValidInCanadaDuringStay: application.haveOrWillHaveHealthInsuranceValidInCanadaDuringStay || '',
                        haveOrWillHaveHealthInsuranceValidInCanadaDuringStayDetails: application.haveOrWillHaveHealthInsuranceValidInCanadaDuringStayDetails || '',
                        inAggreance: '',
                ...
                methods: {
                    isApplyingOnBehalfOfMinorChild: function () {
                        return this.$root.showIsApplyingOnBehalfOfMinorChildField && isYes(this.$root.isApplyingOnBehalfOfMinorChild);
                    },
                    isClientAgeEighteenOrOver: function () {
                        if (this.dobDay !== '' && this.dobMonth !== '' && this.dobYear !== '') {
                            return getDiffInYears(new Date(this.dobYear, this.dobMonth, this.dobDay, 0, 0, 0, 0), new Date()) >= 18;
                        }
                        return false;
                    },
                    otherCitizenshipAddAnother: function () {
                        this.otherCitizenships.push(recordGenerators.otherCitizenship());
                    },
                    otherCitizenshipRemoveAnother: function (otherCitizenship) {
                        this.otherCitizenships.$remove(otherCitizenship);
                    }
                },
                computed: {
                    showEmploymentDetailsSection: function () {
                        return !this.isApplyingOnBehalfOfMinorChild() && this.isClientAgeEighteenOrOver();
                    },
                    showBackgroundQuestionsSection: function () {
                        return !this.isApplyingOnBehalfOfMinorChild() && this.isClientAgeEighteenOrOver();
                    },
                    showOtherCitizenship: function () {
                        return isYes(this.hasOtherCitizenship);
                    },
                    showTaiwanIDNumberField: function () {
                        return this.$root.showHasPassportIssuedByMFAField && isYes(this.$root.hasPassportIssuedByMFA);
                    },
                    showUsaResidentCardNumberField: function () {
                        return this.$root.showIsPermanentResidentOfUSAndHaveProofOfPermResidenceField && isYes(this.$root.isPermanentResidentOfUSAndHaveProofOfPermResidence);
                    },
                    showMaritalStatusField: function () {
                        return this.isClientAgeEighteenOrOver();
                    },
                    showUciField: function () {
                        return isYes(this.hasPreviouslyAppliedToCanada);
                    },
                    occupationJobTitleOptions: function () {
                        return this.employment.occupation === '' ? [] : _.where(jsData.dictionaries ? jsData.dictionaries.jobTitles : [], {
                            occupation_id: this.employment.occupation + ''
                        });
                    },
                    showCompanyEmployerSchoolFacilityNameField: function () {
                        var constants = jsData.constants.employment.occupation;
                        return this.employment.occupation !== '' && !_.contains([
                    ...
                    showProvinceField: function () {
                        var constants = jsData.constants.employment.occupation;
                        return this.employment.occupation !== '' && !_.contains([
                            constants.unemployed,
                            constants.homemaker,
                            constants.retired
                        ], this.employment.occupation + '') && this.employment.country !== '' && this.employment.country == jsData.constants.employment.countries.canada;
                    },
                    showStateField: function () {
                        var constants = jsData.constants.employment.occupation;
                        return this.employment.occupation !== '' && !_.contains([
                            constants.unemployed,
                            constants.homemaker,
                            constants.retired
                        ], this.employment.occupation + '') && this.employment.country !== '' && this.employment.country == jsData.constants.employment.countries.us;
                    },
                    showCityField: function () {
                        var constants = jsData.constants.employment.occupation;
                        return this.employment.occupation !== '' && !_.contains([
                            constants.unemployed,
                            constants.homemaker,
                    ...
                    showAddressProvinceField: function () {
                        return this.country !== '' && this.country == jsData.constants.employment.countries.canada;
                    },
                    showAddressStateField: function () {
                        return this.country !== '' && this.country == jsData.constants.employment.countries.us;
                    },
                    showDistrictField: function () {
                        return this.country !== '' && this.country != jsData.constants.employment.countries.canada && this.country != jsData.constants.employment.countries.us;
                    },
                    showPostalCodeField: function () {
                        return this.country !== '' && this.country == jsData.constants.employment.countries.canada;
                    },
                    showZipCodeField: function () {
                        return this.country !== '' && this.country == jsData.constants.employment.countries.us;
                    },
                    showRefusedVisaOrPermitOrDeniedEntryToCanadaDetailsField: function () {
                        return isYes(this.refusedVisaOrPermitOrDeniedEntryToCanada);
                    },
                    showCommittedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhereField: function () {
                        return !this.isApplyingOnBehalfOfMinorChild();
                    },
                    showCommittedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhereDetailsField: function () {
                        return isYes(this.committedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhere);
                    },
                    showYouOrFamilyMemberInContactWithPersonWithTuberculosisDetailsField: function () {
                        return isYes(this.youOrFamilyMemberInContactWithPersonWithTuberculosis);
                    },
                    showHaveKidneyConditionRequiringDialysisField: function () {
                        return isYes(this.haveHealthConditionReceivingRegularAndOngoingMedicalTreatment);
                    },
                    showTravellingForMedicalReasonsOrTreatmentField: function () {
                        return isNo(this.haveKidneyConditionRequiringDialysis);
                    },
                    showHaveFinancialArrangementsToCoverTreatmentAndRelatedExpensesField: function () {
                        return !this.isApplyingOnBehalfOfMinorChild() && isYes(this.travellingForMedicalReasonsOrTreatment);
                    },
                    showHaveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonthsField: function () {
                        return !this.isApplyingOnBehalfOfMinorChild() && isNo(this.travellingForMedicalReasonsOrTreatment);
                    },
                    showHaveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonthsDetailsField: function () {
                        return isYes(this.haveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonths);
                    },
                    showHaveOrWillHaveHealthInsuranceValidInCanadaDuringStayField: function () {
                        return this.isClientAgeEighteenOrOver() && this.haveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonths !== '';
                    },
                    showBackgroundQuestionsYouMayBeAskedToShowProofOfInsuranceField: function () {
                        return this.isClientAgeEighteenOrOver() && isYes(this.haveOrWillHaveHealthInsuranceValidInCanadaDuringStay);
                    },
                    showBackgroundQuestionsProofOfInsuranceField: function () {
                        return isNo(this.haveHealthConditionReceivingRegularAndOngoingMedicalTreatment) || (isNo(this.haveOrWillHaveHealthInsuranceValidInCanadaDuringStay) && this.isClientAgeEighteenOrOver());
                    }
                },
                watch: {
                    showTaiwanIDNumberField: _.partial(resetFieldIfHidden, 'taiwanIDNumber'),
                    showUsaResidentCardNumberField: _.partial(resetFieldIfHidden, 'usaResidentCardNumber'),
                    showMaritalStatusField: _.partial(resetFieldIfHidden, 'maritalStatus'),
                    showUciField: _.partial(resetFieldIfHidden, 'uci'),
                    occupation: _.partial(resetFieldIfHidden, 'employment.title'),
                    showCompanyEmployerSchoolFacilityNameField: _.partial(resetFieldIfHidden, 'employment.companyEmployerSchoolFacilityName'),
                    showCountryField: _.partial(resetFieldIfHidden, 'employment.country'),
                    showProvinceField: _.partial(resetFieldIfHidden, 'employment.province'),
                    showStateField: _.partial(resetFieldIfHidden, 'employment.state'),
                    showCityField: _.partial(resetFieldIfHidden, 'employment.city'),
                    showFromDate: _.partial(resetFieldIfHidden, 'employment.fromDate'),
                    showAddressProvinceField: _.partial(resetFieldIfHidden, 'province'),
                    showAddressStateField: _.partial(resetFieldIfHidden, 'state'),
                    showDistrictField: _.partial(resetFieldIfHidden, 'district'),
                    showPostalCodeField: _.partial(resetFieldIfHidden, 'postalCode'),
                    showZipCodeField: _.partial(resetFieldIfHidden, 'zipCode'),
                    showRefusedVisaOrPermitOrDeniedEntryToCanadaDetailsField: _.partial(resetFieldIfHidden, 'refusedVisaOrPermitOrDeniedEntryToCanadaDetails'),
                    showCommittedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhereField: _.partial(resetFieldIfHidden, 'committedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhere'),
                    showCommittedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhereDetailsField: _.partial(resetFieldIfHidden, 'committedOrArrestedOrChargedOrConvictedOfCriminalOffenceAnywhereDetails'),
                    showYouOrFamilyMemberInContactWithPersonWithTuberculosisDetailsField: _.partial(resetFieldIfHidden, 'youOrFamilyMemberInContactWithPersonWithTuberculosisDetails'),
                    showHaveKidneyConditionRequiringDialysisField: _.partial(resetFieldIfHidden, 'haveKidneyConditionRequiringDialysis'),
                    showTravellingForMedicalReasonsOrTreatmentField: _.partial(resetFieldIfHidden, 'travellingForMedicalReasonsOrTreatment'),
                    showHaveFinancialArrangementsToCoverTreatmentAndRelatedExpensesField: _.partial(resetFieldIfHidden, 'haveFinancialArrangementsToCoverTreatmentAndRelatedExpenses'),
                    showHaveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonthsField: _.partial(resetFieldIfHidden, 'haveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonths'),
                    showHaveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonthsDetailsField: _.partial(resetFieldIfHidden, 'haveHistoryOfIllnessOrHospitalisationOrInjuryOrSurgeryWithinLastSixMonthsDetails'),
                    showHaveOrWillHaveHealthInsuranceValidInCanadaDuringStayField: _.partial(resetFieldIfHidden, 'haveOrWillHaveHealthInsuranceValidInCanadaDuringStay')
                }
            });

            var representative = application.representative || {};

            var RepresentativeForm = Vue.extend({
                template: $('#template-representative-form').html(),
                components: {
                    'application-form': ApplicationForm
                },
                data: function () {
                    return {
                        representativeRelationshipOptions: jsData.dictionaries ? jsData.dictionaries.representativeRelationships : [],
                        representativeRelationship: representative.representativeRelationship || '',
                        representativeCompensated: representative.representativeCompensated || '',
                        membershipIdNumber: representative.membershipIdNumber || '',
                        ...
                        postalCodeZip: representative.postalCodeZip || '',
                ...
                methods: {
                    isCanadianProvincialOrTerritorialLawSociety: function () {
                        return this.representativeRelationship !== '' && this.representativeRelationship == jsData.constants.representative.relationship.canadianProvincialOrTerritorialLawSociety;
                    }
                },
                computed: {
                    showMembershipIdNumberField: function () {
                        if (this.representativeRelationship === '') {
                            return false;
                        }
                        var constants = jsData.constants.representative.relationship;
                        return _.contains([
                            constants.canadianProvincialOrTerritorialLawSociety,
                            constants.immigrationConsultantOfCanadaRegulatoryCouncil,
                            constants.chambresOfNotairesDuQuebec
                        ], this.representativeRelationship + '');
                    },
                    showProvinceField: function () {
                        return this.isCanadianProvincialOrTerritorialLawSociety();
                    },
                    showOrganizationNameField: function () {
                        return this.representativeRelationship !== '' && this.representativeRelationship != jsData.constants.representative.relationship.familyOrFriend;
                    },
                    showPostalCodeZipField: function () {
                        return this.isCanadianProvincialOrTerritorialLawSociety();
                    }
                },
                watch: {
                    showMembershipIdNumberField: _.partial(resetFieldIfHidden, 'membershipIdNumber'),
                    showProvinceField: _.partial(resetFieldIfHidden, 'province'),
                    showOrganizationNameField: _.partial(resetFieldIfHidden, 'organizationName'),
                    showPostalCodeZipField: _.partial(resetFieldIfHidden, 'postalCodeZip')
                }
            });
            ...
            var app = new Vue({
                el: '#apply-form',
                components: {
                    'not-eligible-result': NotEligibleResult,
                    'exempt-result': ExemptResult,
                    'representative-form': RepresentativeForm,
                    'application-form': ApplicationForm
                },
                data: {
                    citizenshipCountryOptions: jsData.dictionaries ? jsData.dictionaries.citizenshipCountries : [],
                    exemptCountries: _.where(jsData.dictionaries ? jsData.dictionaries.citizenshipCountries : [], {
                        is_exempt: 1
                    }),
                    id: application.id || '',
                    citizenshipCountry: application.citizenshipCountry || '',
                    hasPassportIssuedByMFA: application.hasPassportIssuedByMFA || '',
                    hasIsraeliNationalPassport: application.hasIsraeliNationalPassport || '',
                    isPermanentResidentOfUSAndHaveProofOfPermResidence: application.isPermanentResidentOfUSAndHaveProofOfPermResidence || '',
                    isTravellingByAir: application.isTravellingByAir || '',
                    doesFollowingApply: application.doesFollowingApply || '',
                    doYouStillWantToApply: application.doYouStillWantToApply || '',
                    isRepresentative: application.isRepresentative || '',
                    isApplyingOnBehalfOfMinorChild: application.isApplyingOnBehalfOfMinorChild || ''
                },
                methods: {
                    isTaiwan: function (countryId) {
                        return countryId !== '' && countryId == jsData.constants.countries.taiwan;
                    },
                    isIsrael: function (countryId) {
                        return countryId !== '' && countryId == jsData.constants.countries.israel;
                    },
                    isVisaExempted: function (countryId) {
                        return countryId !== '' && !!_.findWhere(this.exemptCountries, {
                            id: countryId
                        });
                    },
                    getIsRepresentative: function () {
                        return this.showIsRepresentativeField && isYes(this.isRepresentative);
                    },
                    getIsTravellingByAir: function () {
                        return this.showIsTravellingByAirField && isYes(this.isTravellingByAir);
                    },
                    onSubmit: function () {
                        var isValid = true;
                        _.each(validations, function (validation) {
                            isValid = validation.validate(true) && isValid;
                        });
                        if (!isValid) {
                            /*var errors = [];
                            var errorMessages = jsData.translations.errors;
                            var index = 1;
                            $('.has-error').each(function () {
                                var el = $(this);
                                var errorType = el.data('error-mode') || 'required';
                                var name = el.attr('name');
                                var error = '';
                                if (errorMessages.hasOwnProperty(name) && errorMessages[name].hasOwnProperty(errorType)) {
                                    error = $.trim(errorMessages[name][errorType]);
                                } else {
                                    if (/_(day|month|year)$/.test(name)) {
                                        name = name.replace(/_(day|month|year)$/, '');
                                        if (errorMessages.hasOwnProperty(name) && errorMessages[name].hasOwnProperty(errorType)) {
                                            error = $.trim(errorMessages[name][errorType]);
                                        }
                                    }
                            ...
                            if (errors.length) {
                                showAlert(jsData.translations.errors_title + ':<br/><ol><li>' + errors.join('</li><li>') + '</li></ol>');
                            }
                            return false;*/
                        } else {
                        ...
                        if (isValid) {
                            var nextStep = this.$refs.nextStep;
                            var data = JSON.parse(JSON.stringify(this.$data));
                            isValid = false;
                            if (nextStep instanceof ApplicationForm) {
                                _.extend(data, JSON.parse(JSON.stringify(nextStep.$data)));
                                isValid = true;
                            } else {
                                if (nextStep instanceof RepresentativeForm && nextStep.$refs.applicationForm instanceof  ApplicationForm) {
                                    _.extend(data, JSON.parse(JSON.stringify(nextStep.$refs.applicationForm.$data)));
                                    data.representative = JSON.parse(JSON.stringify(nextStep.$data));
                                    isValid = true;
                                }
                            }
                            if (isValid) {
                                var cleanup = function (value, key, target) {
                                    if (_.contains(['currentYear', 'exemptCountries'], key) || /Options$/.test(key)) {
                                        delete target[key];
                                    } else {
                                        if (_.isObject(value)) {
                                            _.each(value, cleanup);
                                        }
                                    }
                                };
                                _.each(data, cleanup);
                                data.action = 'application_save';
                ...
                computed: {
                    showHasPassportIssuedByMFAField: function () {
                        return this.isTaiwan(this.citizenshipCountry);
                    },
                    showHasIsraeliNationalPassportField: function () {
                        return this.isIsrael(this.citizenshipCountry);
                    },
                    showIsPermanentResidentOfUSAndHaveProofOfPermResidenceField: function () {
                        if (this.citizenshipCountry === '') {
                            return false;
                        }
                        if (this.showHasPassportIssuedByMFAField) {
                            return isNo(this.hasPassportIssuedByMFA);
                        } else {
                            if (this.showHasIsraeliNationalPassportField) {
                                return isNo(this.hasIsraeliNationalPassport);
                            }
                        }
                        return !this.isVisaExempted(this.citizenshipCountry);
                    },
                    showIsTravellingByAirField: function () {
                        if (this.citizenshipCountry === '') {
                            return false;
                        }
                        var isVisaExempted = this.isVisaExempted(this.citizenshipCountry);
                        return (!isVisaExempted && ((this.showIsPermanentResidentOfUSAndHaveProofOfPermResidenceField && isYes(this.isPermanentResidentOfUSAndHaveProofOfPermResidence)) || (this.showHasPassportIssuedByMFAField && isYes(this.hasPassportIssuedByMFA)) || (this.showHasIsraeliNationalPassportField && isYes(this.hasIsraeliNationalPassport)))) || isVisaExempted;
                    },
                    showDoesFollowingApplyField: function () {
                        return this.getIsTravellingByAir();
                    },
                    showDoYouStillWantToApplyField: function () {
                        if (!this.showDoesFollowingApplyField) {
                            return false;
                        }
                        return isYes(this.doesFollowingApply);
                    },
                    showIsRepresentativeField: function () {
                        return (this.showDoesFollowingApplyField && isNo(this.doesFollowingApply)) || (this.showDoYouStillWantToApplyField && isYes(this.doYouStillWantToApply));
                    },
                    showIsApplyingOnBehalfOfMinorChildField: function () {
                        return this.getIsRepresentative();
                    },
                    isPrerequisiteFormValid: function () {
                        return this.citizenshipCountry !== '' && (!this.showHasPassportIssuedByMFAField || this.hasPassportIssuedByMFA !== '') && (!this.showHasIsraeliNationalPassportField || this.hasIsraeliNationalPassport !== '') && (!this.showIsPermanentResidentOfUSAndHaveProofOfPermResidenceField || this.isPermanentResidentOfUSAndHaveProofOfPermResidence !== '') && (!this.showIsTravellingByAirField || this.isTravellingByAir !== '') && (!this.showDoesFollowingApplyField || this.doesFollowingApply !== '') && (!this.showDoYouStillWantToApplyField || this.doYouStillWantToApply !== '') && (!this.showIsRepresentativeField || this.isRepresentative !== '') && (!this.showIsApplyingOnBehalfOfMinorChildField || this.isApplyingOnBehalfOfMinorChild !== '');
                    },
                    nextStep: function () {
                        if (this.isTaiwan(this.citizenshipCountry)) {
                            if ((this.showHasPassportIssuedByMFAField && isNo(this.hasPassportIssuedByMFA)) && (this.showIsPermanentResidentOfUSAndHaveProofOfPermResidenceField && isNo(this.isPermanentResidentOfUSAndHaveProofOfPermResidence))) {
                                return 'not-eligible-result';
                            }
                        } else {
                            if (this.isIsrael(this.citizenshipCountry)) {
                                if ((this.showHasIsraeliNationalPassportField && isNo(this.hasIsraeliNationalPassport)) && (this.showIsPermanentResidentOfUSAndHaveProofOfPermResidenceField && isNo(this.isPermanentResidentOfUSAndHaveProofOfPermResidence))) {
                                    return 'not-eligible-result';
                                }
                            } else {
                                if (this.showIsPermanentResidentOfUSAndHaveProofOfPermResidenceField && isNo(this.isPermanentResidentOfUSAndHaveProofOfPermResidence)) {
                                    return 'not-eligible-result';
                                }
                            }
                        }
                        var result = 'application-form';
                        if (this.getIsRepresentative()) {
                            result = 'representative-form';
                        }
                        if ((this.showIsTravellingByAirField && !this.getIsTravellingByAir()) || (this.showDoYouStillWantToApplyField && isNo(this.doYouStillWantToApply))) {
                ...
                watch: {
                    showHasPassportIssuedByMFAField: _.partial(resetFieldIfHidden, 'hasPassportIssuedByMFA'),
                    showHasIsraeliNationalPassportField: _.partial(resetFieldIfHidden, 'hasIsraeliNationalPassport'),
                    showIsPermanentResidentOfUSAndHaveProofOfPermResidenceField: _.partial(resetFieldIfHidden, 'isPermanentResidentOfUSAndHaveProofOfPermResidence'),
                    showIsTravellingByAirField: _.partial(resetFieldIfHidden, 'isTravellingByAir'),
                    showDoesFollowingApplyField: _.partial(resetFieldIfHidden, 'doesFollowingApply'),
                    showDoYouStillWantToApplyField: _.partial(resetFieldIfHidden, 'doYouStillWantToApply'),
                    showIsRepresentativeField: _.partial(resetFieldIfHidden, 'isRepresentative'),
                    showIsApplyingOnBehalfOfMinorChildField: _.partial(resetFieldIfHidden, 'isApplyingOnBehalfOfMinorChild')
                },
                created: function () {
                    Vue.util.defineReactive(this, '$validator', validatorScope);
                }
            });
        }
    })();
